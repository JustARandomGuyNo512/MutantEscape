...
import mutantescape.level.register.Particle.InverseFunction;
import mutantescape.level.register.Particle.Process;
...
public class BloodParticle extends TextureSheetParticle {
    ...
    public void tick() {
        ...
        if (this.onGround) {
            ...
            if (this.age < i) {
                this.age = i;
                this.level.addParticle(RegisterParticleTypes.BLOOD_SPLAT.get(), this.x, this.y + 0.1D, this.z, 0.0D, 0.0D, 0.0D);
                for (int j = 3; j-- > 0; ) {
                    this.level.addParticle(RegisterParticleTypes.BLOOD_SPLASH.get(), this.x, this.y, this.z, 0.0D, 0.0D, 0.0D);
                }
